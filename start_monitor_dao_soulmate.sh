#!/bin/bash
# 自动生成的Monitor启动脚本 - DAOSoulMate
#
# 使用方法:
#   ./start_monitor_dao_soulmate.sh
#
# 环境变量（可选）:
...
# 协议配置
PROTOCOL="dao_soulmate"
PROTOCOL_NAME="DAOSoulMate"
DEPLOYMENT_FILE="$PROJECT_ROOT/test/DAO_SoulMate_exp/scripts/data/deployment.json"
TEMPLATE_FILE="config/templates/dao_soulmate_dynamic.json.tmpl"
CONFIG_FILE="pkg/invariants/configs/dao_soulmate-dynamic.json"
RULE_EXPORT_PATH="../test/DAO_SoulMate_exp/scripts/data/firewall-rules.json"
...
if [ ! -f "$DEPLOYMENT_FILE" ]; then
    ...
    echo ""
    echo "请先运行部署脚本："
    echo "  forge script test/DAO_SoulMate_exp/scripts/DeployContracts.s.sol \\"
    echo "    --tc DeployDAOSoulMateContracts \\"
