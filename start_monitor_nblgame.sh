#!/bin/bash
# 自动生成的Monitor启动脚本 - NBLGAME
#
# 使用方法:
#   ./start_monitor_nblgame.sh
#
# 环境变量（可选）:
...
# 协议配置
PROTOCOL="nblgame"
PROTOCOL_NAME="NBLGAME"
DEPLOYMENT_FILE="$PROJECT_ROOT/test/NBLGAME_exp/scripts/data/deployment.json"
TEMPLATE_FILE="config/templates/nblgame_dynamic.json.tmpl"
CONFIG_FILE="pkg/invariants/configs/nblgame-dynamic.json"
RULE_EXPORT_PATH="../test/NBLGAME_exp/scripts/data/firewall-rules.json"
...
if [ ! -f "$DEPLOYMENT_FILE" ]; then
    ...
    echo ""
    echo "请先运行部署脚本："
    echo "  forge script test/NBLGAME_exp/scripts/DeployContracts.s.sol \\"
    echo "    --tc DeployNBLGAMEContracts \\"
