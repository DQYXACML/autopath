#!/bin/bash
# 自动生成的Monitor启动脚本 - RadiantCapital
#
# 使用方法:
#   ./start_monitor_radiantcapital.sh
#
# 环境变量（可选）:
...
PROTOCOL_NAME="RadiantCapital"
DEPLOYMENT_FILE="$PROJECT_ROOT/test/RadiantCapital_exp/scripts/data/deployment.json"
TEMPLATE_FILE="config/templates/radiantcapital_dynamic.json.tmpl"
CONFIG_FILE="pkg/invariants/configs/radiantcapital-dynamic.json"
RULE_EXPORT_PATH="../test/RadiantCapital_exp/scripts/data/firewall-rules.json"
...
if [ ! -f "$DEPLOYMENT_FILE" ]; then
    ...
    echo ""
    echo "请先运行部署脚本："
    echo "  forge script test/RadiantCapital_exp/scripts/DeployContracts.s.sol \\"
    echo "    --tc DeployRadiantCapitalContracts \\"
