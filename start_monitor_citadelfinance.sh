#!/bin/bash
# 自动生成的Monitor启动脚本 - CitadelFinance
#
# 使用方法:
#   ./start_monitor_citadelfinance.sh
#
# 环境变量（可选）:
...
# 协议配置
PROTOCOL="citadelfinance"
PROTOCOL_NAME="CitadelFinance"
DEPLOYMENT_FILE="$PROJECT_ROOT/test/CitadelFinance_exp/scripts/data/deployment.json"
TEMPLATE_FILE="config/templates/citadelfinance_dynamic.json.tmpl"
CONFIG_FILE="pkg/invariants/configs/citadelfinance-dynamic.json"
RULE_EXPORT_PATH="../test/CitadelFinance_exp/scripts/data/firewall-rules.json"
...
if [ ! -f "$DEPLOYMENT_FILE" ]; then
    ...
    echo ""
    echo "请先运行部署脚本："
    echo "  forge script test/CitadelFinance_exp/scripts/DeployContracts.s.sol \\"
    echo "    --tc DeployCitadelFinanceContracts \\"
