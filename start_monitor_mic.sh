#!/bin/bash
# 自动生成的Monitor启动脚本 - MIC
#
# 使用方法:
#   ./start_monitor_mic.sh
#
# 环境变量（可选）:
...
PROTOCOL="mic"
PROTOCOL_NAME="MIC"
DEPLOYMENT_FILE="$PROJECT_ROOT/test/MIC_exp/scripts/data/deployment.json"
TEMPLATE_FILE="config/templates/mic_dynamic.json.tmpl"
CONFIG_FILE="pkg/invariants/configs/mic-dynamic.json"
RULE_EXPORT_PATH="../test/MIC_exp/scripts/data/firewall-rules.json"
...
if [ ! -f "$DEPLOYMENT_FILE" ]; then
    ...
    echo ""
    echo "请先运行部署脚本："
    echo "  forge script test/MIC_exp/scripts/DeployContracts.s.sol \\"
    echo "    --tc DeployMICContracts \\"
