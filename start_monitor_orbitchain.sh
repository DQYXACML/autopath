#!/bin/bash
# 自动生成的Monitor启动脚本 - OrbitChain
#
# 使用方法:
#   ./start_monitor_orbitchain.sh
#
# 环境变量（可选）:
...
# 协议配置
PROTOCOL="orbitchain"
PROTOCOL_NAME="OrbitChain"
DEPLOYMENT_FILE="$PROJECT_ROOT/test/OrbitChain_exp/scripts/data/deployment.json"
TEMPLATE_FILE="config/templates/orbitchain_dynamic.json.tmpl"
CONFIG_FILE="pkg/invariants/configs/orbitchain-dynamic.json"
RULE_EXPORT_PATH="../test/OrbitChain_exp/scripts/data/firewall-rules.json"
...
if [ ! -f "$DEPLOYMENT_FILE" ]; then
    ...
    echo ""
    echo "请先运行部署脚本："
    echo "  forge script test/OrbitChain_exp/scripts/DeployContracts.s.sol \\"
    echo "    --tc DeployOrbitChainContracts \\"
