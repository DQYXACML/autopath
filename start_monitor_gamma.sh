#!/bin/bash
# 自动生成的Monitor启动脚本 - Gamma
#
# 使用方法:
#   ./start_monitor_gamma.sh
#
# 环境变量（可选）:
...
PROTOCOL="gamma"
PROTOCOL_NAME="Gamma"
DEPLOYMENT_FILE="$PROJECT_ROOT/test/Gamma_exp/scripts/data/deployment.json"
TEMPLATE_FILE="config/templates/gamma_dynamic.json.tmpl"
CONFIG_FILE="pkg/invariants/configs/gamma-dynamic.json"
RULE_EXPORT_PATH="../test/Gamma_exp/scripts/data/firewall-rules.json"
...
if [ ! -f "$DEPLOYMENT_FILE" ]; then
    ...
    echo ""
    echo "请先运行部署脚本："
    echo "  forge script test/Gamma_exp/scripts/DeployContracts.s.sol \\"
    echo "    --tc DeployGammaContracts \\"
