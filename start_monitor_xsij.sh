#!/bin/bash
# 自动生成的Monitor启动脚本 - XSIJ
#
# 使用方法:
#   ./start_monitor_xsij.sh
#
# 环境变量（可选）:
...
PROTOCOL="xsij"
PROTOCOL_NAME="XSIJ"
DEPLOYMENT_FILE="$PROJECT_ROOT/test/XSIJ_exp/scripts/data/deployment.json"
TEMPLATE_FILE="config/templates/xsij_dynamic.json.tmpl"
CONFIG_FILE="pkg/invariants/configs/xsij-dynamic.json"
RULE_EXPORT_PATH="../test/XSIJ_exp/scripts/data/firewall-rules.json"
...
if [ ! -f "$DEPLOYMENT_FILE" ]; then
    ...
    echo ""
    echo "请先运行部署脚本："
    echo "  forge script test/XSIJ_exp/scripts/DeployContracts.s.sol \\"
    echo "    --tc DeployXSIJContracts \\"
