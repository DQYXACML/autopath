#!/bin/bash
# 自动生成的Monitor启动脚本 - BarleyFinance
#
# 使用方法:
#   ./start_monitor_barleyfinance.sh
#
# 环境变量（可选）:
...
PROTOCOL_NAME="BarleyFinance"
DEPLOYMENT_FILE="$PROJECT_ROOT/test/BarleyFinance_exp/scripts/data/deployment.json"
TEMPLATE_FILE="config/templates/barleyfinance_dynamic.json.tmpl"
CONFIG_FILE="pkg/invariants/configs/barleyfinance-dynamic.json"
RULE_EXPORT_PATH="../test/BarleyFinance_exp/scripts/data/firewall-rules.json"
...
if [ ! -f "$DEPLOYMENT_FILE" ]; then
    ...
    echo ""
    echo "请先运行部署脚本："
    echo "  forge script test/BarleyFinance_exp/scripts/DeployContracts.s.sol \\"
    echo "    --tc DeployBarleyFinanceContracts \\"
