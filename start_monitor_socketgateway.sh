#!/bin/bash
# 自动生成的Monitor启动脚本 - SocketGateway
#
# 使用方法:
#   ./start_monitor_socketgateway.sh
#
# 环境变量（可选）:
...
# 协议配置
PROTOCOL="socketgateway"
PROTOCOL_NAME="SocketGateway"
DEPLOYMENT_FILE="$PROJECT_ROOT/test/SocketGateway_exp/scripts/data/deployment.json"
TEMPLATE_FILE="config/templates/socketgateway_dynamic.json.tmpl"
CONFIG_FILE="pkg/invariants/configs/socketgateway-dynamic.json"
RULE_EXPORT_PATH="../test/SocketGateway_exp/scripts/data/firewall-rules.json"
...
if [ ! -f "$DEPLOYMENT_FILE" ]; then
    ...
    echo ""
    echo "请先运行部署脚本："
    echo "  forge script test/SocketGateway_exp/scripts/DeployContracts.s.sol \\"
    echo "    --tc DeploySocketGatewayContracts \\"
