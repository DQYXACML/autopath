#!/bin/bash
# 自动生成的Monitor启动脚本 - Freedom
#
# 使用方法:
#   ./start_monitor_freedom.sh
#
# 环境变量（可选）:
...
# 协议配置
PROTOCOL="freedom"
PROTOCOL_NAME="Freedom"
DEPLOYMENT_FILE="$PROJECT_ROOT/test/Freedom_exp/scripts/data/deployment.json"
TEMPLATE_FILE="config/templates/freedom_dynamic.json.tmpl"
CONFIG_FILE="pkg/invariants/configs/freedom-dynamic.json"
RULE_EXPORT_PATH="../test/Freedom_exp/scripts/data/firewall-rules.json"
...
if [ ! -f "$DEPLOYMENT_FILE" ]; then
    ...
    echo ""
    echo "请先运行部署脚本："
    echo "  forge script test/Freedom_exp/scripts/DeployContracts.s.sol \\"
    echo "    --tc DeployFreedomContracts \\"
